...
FlightDelay & Augmented & $0.53$ & 0.495 & Random \\
\bottomrule
\end{tabular}%
}
\end{table}
...
Across all 10 datasets, our framework achieves positive or zero weighted improvement, demonstrating that incremental learning without data sharing incurs no performance loss compared to standard training (Wilcoxon $p = 0.002$). The strongest gains appear in BankLoanSta (+0.0294), HRAnalytics (+0.0049), ClientRecord (+0.0031), and MovieAugV2 (+0.0028).
...
\textbf{Statistical significance.} All 10 datasets show positive or zero weighted improvement. A Wilcoxon signed-rank test confirms statistical significance ($W = 0$, $p = 0.004$). The median weighted improvement is +0.0024 AUC.
...
FlightDelay & \textbf{--0.0021} & --0.0019 & +0.0027 & Optuna & 0.7188 \\
\midrule
