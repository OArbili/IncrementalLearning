...
Evaluation across 10 diverse datasets demonstrates that incremental learning without data sharing achieves near-parity with standard training, with all 10 datasets showing positive or zero weighted improvement (Wilcoxon $p = 0.001$). In addition, we benchmark against five state-of-the-art baselines for learning under evolving feature spaces --- AdaptiveXGBoost, PUFE, OCDS, EMLI, and GBDT-IL --- under an identical preprocessing and data-split protocol. Our framework wins on \emph{all 10 datasets} against every baseline, exceeding the closest competitor (GBDT-IL) by $+0.041$ mean population-weighted AUC.
\end{abstract}
...
\item \textbf{Comprehensive benchmarking against the state of the art:} We re-implement and evaluate five recent baselines for learning with evolving or capricious feature spaces --- AdaptiveXGBoost~\cite{montiel2020axgb}, PUFE~\cite{he2021pufe}, OCDS~\cite{he2019ocds}, EMLI~\cite{dong2021eml}, and GBDT-IL~\cite{zhang2019gbdtil} --- under a single shared preprocessing and stratified train/test split so numbers are directly comparable. Our framework outperforms every baseline on every dataset (10/10), with a mean population-weighted AUC margin of $+0.041$ over the closest competitor (GBDT-IL) and up to $+0.14$ over the weakest (OCDS).
\end{itemize}
...
\centering
 \caption{
    Datasets used in experiments. Label\% shows positive class proportion. Null\% is the percentage of rows where at least one extended feature is missing, i.e., the fraction of the population that lacks extended information and relies solely on the base model.
  }
  ...
  \begin{tabular}{l r r r r}
    ...
    \midrule
    FlightDelay        & 100,000  & 14  & 17.3 & 30.0 \\
    WeatherAUS         & 142,193  & 20  & 22.4 & 40.3 \\
    MovieAugV2         & 112,466  & 11  & 1.9  & 88.5 \\
    DiabetesRecord     & 101,766  & 45  & 46.1 & 66.2 \\
    BankLoanSta        & 100,000  & 17  & 77.4 & 62.7 \\
    WIDS               & 91,713   & 51  & 8.6  & 87.4 \\
    CreditRisk         & 80,000   & 41  & 8.0  & 89.6 \\
    Weather            & 25,000   & 16  & 22.4 & 62.3 \\
    HRAnalytics        & 19,158   & 14  & 24.9 & 31.0 \\
    ClientRecord       & 7,043    & 28  & 67.0 & 40.0 \\
  \bottomrule
  \end{tabular}%
  }
