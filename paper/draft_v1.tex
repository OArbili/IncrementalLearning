...
Through rigorous evaluation on 10 diverse real-world binary classification datasets, our method consistently outperforms full retrain approaches across all evaluated datasets. Moreover, our framework offers operational advantages including simplified model maintenance and graceful handling of partial feature availability at inference time. Notably, the two-phase design directly enables privacy-constrained collaboration: our experimental setup mirrors a scenario where Institution A trains a base model on its available features and shares only the model artifact, while Institution B extends it with additional features from a different subpopulation --- without either party accessing the other's raw data. All 10 evaluated datasets demonstrate this capability, achieving consistent improvements through model-level knowledge transfer. This approach provides a practical solution for organizations deploying predictive models in dynamic, evolving data environments with structured missingness, as well as for cross-institutional collaboration under data-sharing restrictions.
\end{abstract}
...
In this work, we introduce a feature-based incremental learning framework for Gradient Boosted Machines (GBMs) that addresses both evolving feature spaces and privacy-constrained model collaboration. Our approach enables models to evolve alongside feature availability while preserving performance, model maintainability, and deployment simplicity. We validate our framework on 10 diverse datasets spanning healthcare, finance, and meteorology domains, demonstrating consistent improvements over full retrain baselines across all evaluated datasets.
...
\subsection{Why Two-Phase Outperforms Heuristic Missing-Value Handling}

Our results provide insight into why explicit population-aware modeling outperforms the heuristic missing-value mechanisms built into modern GBMs. The combined baseline (a single XGBoost model trained on all features) relies on learned default split directions to handle missing extended features. While effective for random missingness, this approach has a fundamental limitation under structured missingness: it must use the same tree structure to serve both the population with extended features and the population without. Our analysis (Section~5.3) reveals that the base model outperforms the combined baseline on the no-extended population in all 10 datasets, indicating that the combined model's heuristic routing dilutes its predictive power for this subgroup. By training a dedicated base model on this population, our framework enables specialization that a single model with default-direction routing cannot achieve.

This finding has broader implications: when missingness is structured and predictable (not random), the missingness pattern itself is informative, and models benefit from explicitly conditioning on feature availability rather than treating absence as a nuisance to be routed around.
...
Beyond feature evolution, our experiments directly demonstrate a privacy-constrained collaboration paradigm, where institutions share model artifacts rather than raw data. All 10 datasets validate that this model-level knowledge transfer achieves consistent improvements without cross-phase data access.
