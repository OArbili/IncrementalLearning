...
\item \textbf{Comprehensive benchmarking:} We re-implement and evaluate five recent baselines for learning with evolving or capricious feature spaces - AdaptiveXGBoost~\cite{montiel2020axgb}, PUFE~\cite{he2021pufe}, OCDS~\cite{he2019ocds}, EMLI~\cite{dong2021eml}, and GBDT-IL~\cite{zhang2019gbdtil} - under a single shared preprocessing and stratified train/test split so numbers are directly comparable. Our framework outperforms every baseline on every dataset (10/10).
...
GBDT-IL~\cite{zhang2019gbdtil} & 0.7657 & $-0.0412$ & 10/10 \\
...
A head-to-head comparison against five recent baselines for learning with evolving feature spaces - AdaptiveXGBoost~\cite{montiel2020axgb}, PUFE~\cite{he2021pufe}, OCDS~\cite{he2019ocds}, EMLI~\cite{dong2021eml}, and GBDT-IL~\cite{zhang2019gbdtil} - run under an identical preprocessing and data-split protocol shows that our framework wins on \emph{all 10 datasets} against \emph{every} baseline. The mean population-weighted AUC margin is $+0.041$ over the closest competitor (GBDT-IL, which shares our GBDT backbone) and up to $+0.14$ over the weakest (OCDS), isolating the contribution of the $\{$base, extended$\}$ model decomposition beyond the choice of learner.
