...
\begin{table*}[!t]
\caption{AUROC Results Across All Datasets. Our model = two-phase incremental approach; Baseline = single combined model trained on all features. Objective measures total improvement: negative values indicate our method outperforms the baseline.}
\label{tab:merged_auc_results}
\centering
\small
\begin{tabular}{l r r c c c c c}
\toprule
\textbf{Dataset} & \textbf{Rows} & \textbf{Null\%} & \textbf{Our (base)} & \textbf{Baseline (base)} & \textbf{Our (ext)} & \textbf{Baseline (ext)} & \textbf{Objective} \\
\midrule
...
WeatherAUS & 142K & 40.1 & 0.8923 & 0.8906 & 0.9016 & 0.9021 & \textcolor{ForestGreen}{--0.0011} \\
