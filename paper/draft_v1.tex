...
\item \textbf{Adaptive tree pruning for knowledge transfer:} We develop a validation-driven pruning mechanism that determines how much prior knowledge to retain when extending to new feature spaces. Bayesian optimization selects the optimal pruning level, with a built-in $k=0$ fallback that guarantees the framework never underperforms training from scratch. Adaptive pruning outperforms fixed strategies in 6 of 10 datasets.
...
WIDS & --0.0007 & \textbf{--0.0009} & --0.0002 & No Pruning & 0.8967 \\
FlightDelay & \textbf{--0.0021} & --0.0019 & +0.0027 & Optuna & 0.7188 \\
\midrule
\multicolumn{4}{l}{\textbf{Wins}} & \multicolumn{2}{l}{Optuna: 6, No Pruning: 4, Fixed 50\%: 0} \\
\bottomrule
\end{tabular}%
}
\end{table}
