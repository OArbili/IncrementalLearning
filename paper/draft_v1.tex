...
We introduce a population-aware learning framework for Gradient Boosting Machines (GBMs) that explicitly models structured missingness by training specialized models for distinct subpopulations defined by feature availability. Our approach combines three key innovations: (1) strategic feature allocation that partitions features into base and extended sets based on availability patterns; (2) adaptive model transfer with validation-driven tree pruning that preserves valuable knowledge while minimizing negative transfer; and (3) population-specific specialization, where a base model serves instances lacking extended features while a warm-started extended model serves the enriched subpopulation.
...
In this work, we introduce a population-aware learning framework for Gradient Boosted Machines (GBMs) that treats structured missingness as a modeling opportunity rather than a nuisance. Instead of training a single model that heuristically routes around missing values, our approach explicitly trains specialized models for subpopulations defined by feature availability. We validate our framework on 10 diverse datasets spanning healthcare, finance, and meteorology domains, demonstrating statistically significant improvements (Wilcoxon $p = 0.002$) over combined baselines across all evaluated datasets.
Our main contributions are:
\begin{itemize}
\setlength\itemindent{0pt}
\setlength{\itemindent}{0pt}
\item Population-aware modeling under structured missingness: We demonstrate that explicitly partitioning training into subpopulation-specific phases --- rather than relying on heuristic missing-value routing --- yields consistent improvements across 10 diverse datasets. Our analysis reveals that the primary benefit comes from the base model specializing on the feature-absent population, a subgroup that combined models systematically underserve.

\item Adaptive tree pruning for knowledge transfer: We develop a validation-driven mechanism to selectively retain useful base trees when extending to new feature spaces. Bayesian optimization determines the optimal pruning level, including a $k=0$ fallback that guarantees the framework never underperforms training from scratch.

\item Privacy-preserving model collaboration: Our experimental design directly validates a model-sharing paradigm where institutions exchange trained model artifacts rather than raw data. All 10 datasets demonstrate successful cross-phase knowledge transfer without data access between phases, providing empirical evidence for privacy-constrained deployment.
\end{itemize}
