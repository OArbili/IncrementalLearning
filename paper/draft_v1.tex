...
Evaluation across 10 diverse datasets demonstrates that incremental learning without data sharing achieves near-parity with standard training, with all 10 datasets showing positive or zero weighted improvement (Wilcoxon $p = 0.002$).
\end{abstract}
...
In this work, we introduce an incremental learning framework for GBMs that integrates new features without full retraining or data sharing, and demonstrate that it achieves near-parity with standard training across 10 diverse datasets (Wilcoxon $p = 0.002$), with all 10 datasets showing positive or zero improvement. Our contributions are:
\begin{itemize}
...
Across all 10 datasets, our framework achieves positive or zero weighted improvement, demonstrating that incremental learning without data sharing incurs no performance loss compared to standard training (Wilcoxon $p = 0.002$). The strongest gains appear in BankLoanSta (+0.0294), HRAnalytics (+0.0049), FlightDelay (+0.0035), and ClientRecord (+0.0031).
...
\textbf{Statistical significance.} All 10 datasets show positive or zero weighted improvement. A Wilcoxon signed-rank test confirms statistical significance ($W = 0$, $p = 0.004$). The median weighted improvement is +0.0027 AUC.
...
CreditRisk & 80K & 95.0 & 0.7540 & 0.7536 & 0.7563 & 0.7569 & \textcolor{ForestGreen}{+0.0002} \\
