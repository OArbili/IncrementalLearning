...
Evaluation across 10 diverse datasets confirms statistically significant positive improvements (Wilcoxon $p = 0.002$), demonstrating that incremental learning without data sharing achieves at least parity with standard training --- and often surpasses it.
\end{abstract}
...
Across all 10 datasets, our framework achieves positive total improvement, demonstrating that incremental learning without data sharing incurs no performance loss compared to standard training --- and in fact yields consistent, statistically significant gains. The strongest gains appear in BankLoanSta (+0.0743), Weather (+0.0097), FlightDelay (+0.0046), and ClientRecord (+0.0046).
...
\textbf{Statistical significance.} All 10 datasets show positive total improvement. A Wilcoxon signed-rank test confirms that the improvements are statistically significant ($W = 0$, $p = 0.002$), indicating that the probability of all 10 datasets showing positive improvement by chance is less than 0.2\%. The median improvement across datasets is +0.0025 AUC.
...
ClientRecord & 7K & 47.5 & 0.9235 & 0.9198 & 0.9186 & 0.9177 & \textcolor{ForestGreen}{+0.0046} \\
