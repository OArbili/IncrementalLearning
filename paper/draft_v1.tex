...
Through rigorous evaluation on 10 diverse real-world binary classification datasets, our method consistently outperforms full retrain approaches across all evaluated datasets. Moreover, our framework offers operational advantages including simplified model maintenance and graceful handling of partial feature availability at inference time. Crucially, the two-phase design also enables a privacy-preserving model collaboration paradigm: institutions can share trained model artifacts rather than raw data, allowing cross-organizational knowledge transfer under regulatory constraints such as GDPR and HIPAA. This approach provides a practical solution for organizations deploying predictive models in dynamic, evolving data environments with structured missingness.
\end{abstract}
...
A closely related challenge arises in cross-organizational collaboration, where multiple institutions hold complementary feature sets but cannot share raw data due to privacy regulations. For example, two hospitals may collect different diagnostic measurements for overlapping patient populations, or banks collaborating on anti-money laundering (AML) models may hold transaction features that cannot be exchanged. Our framework naturally addresses this scenario: one institution trains a base model and shares only the model artifact, while the other extends it with private features --- enabling knowledge transfer without data exposure.

In this work, we introduce a feature-based incremental learning framework for Gradient Boosted Machines (GBMs) that addresses both feature evolution and privacy-constrained collaboration. Our approach enables models to evolve alongside feature availability while preserving performance, model maintainability, and deployment simplicity. We validate our framework on 10 diverse datasets spanning healthcare, finance, and meteorology domains, demonstrating consistent improvements over full retrain baselines across all evaluated datasets.
...
\subsection{Application to Privacy-Preserving Model Collaboration}

Beyond feature evolution within a single organization, our framework naturally extends to scenarios where data sharing is restricted by privacy regulations. Consider two hospitals that each collect different diagnostic measurements for overlapping patient populations, or banks collaborating on anti-money laundering (AML) models where transaction features from one institution cannot be shared with another due to regulatory constraints such as GDPR or HIPAA.

In such settings, our two-phase approach offers a practical alternative to federated learning or secure multi-party computation. Institution A trains a base model on its available features and shares only the model artifact (not the underlying data). Institution B then extends this model with its private features using our warm-start and adaptive pruning mechanism. The resulting extended model leverages knowledge from both institutions without either party exposing raw data. This model-sharing paradigm preserves data sovereignty while enabling cross-institutional knowledge transfer --- a capability that is increasingly important as regulatory frameworks tighten around data portability and privacy.
...
Beyond feature evolution, the framework offers a practical paradigm for privacy-preserving model collaboration, enabling institutions to share model artifacts rather than raw data when combining complementary feature sets under regulatory constraints.
