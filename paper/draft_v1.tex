...
Evaluation across 10 diverse datasets demonstrates that incremental learning without data sharing achieves near-parity with standard training (Wilcoxon $p = 0.074$), with 8 of 10 datasets showing positive weighted improvement and the remaining 2 showing negligible deficits below 0.001 AUC.
\end{abstract}
...
In this work, we introduce an incremental learning framework for GBMs that integrates new features without full retraining or data sharing, and demonstrate that it achieves near-parity with standard training across 10 diverse datasets (Wilcoxon $p = 0.074$), with 8 of 10 datasets showing positive improvement. Our contributions are:
\begin{itemize}
...
Across 10 datasets, our framework achieves positive or near-zero weighted improvement in all cases, demonstrating that incremental learning without data sharing incurs negligible performance difference compared to standard training --- and in 8 of 10 datasets yields measurable gains. The strongest gains appear in BankLoanSta (+0.0743), Weather (+0.0097), FlightDelay (+0.0046), and ClientRecord (+0.0046).
...
For the extended population, results are more nuanced. In 6 out of 10 datasets, the extended model matches or exceeds the combined baseline on extended-feature rows. In the remaining 3 datasets (Weather, HRAnalytics, WeatherAUS), the extended model shows marginal deficits (less than 0.002 AUC), which are more than compensated by the base model's gains on the no-extended population.

\textbf{Statistical significance.} Using population-weighted improvement, 8 of 10 datasets show positive gains, with 2 datasets (Weather and HRAnalytics) showing marginal deficits below 0.001 AUC. A Wilcoxon signed-rank test yields $W = 7$, $p = 0.074$, indicating a trend toward positive improvement. The median weighted improvement is +0.0006 AUC, and the mean is +0.0035 AUC (skewed by BankLoanSta's strong gain).
...
Weather & 25K & 49.3 & 0.8432 & 0.8314 & 0.8448 & 0.8469 & \textcolor{red}{--0.0005} \\
...
HRAnalytics & 19K & 23.5 & 0.7735 & 0.7727 & 0.8102 & 0.8108 & \textcolor{red}{--0.0003} \\
...
Evaluation across 10 diverse datasets from healthcare, finance, and meteorology domains demonstrates that incremental learning without data sharing achieves near-parity with standard training, with weighted AUC improvements ranging from --0.0005 to +0.0294 across 10 datasets (Wilcoxon $p = 0.074$, 8 of 10 positive). This is the central result: \textit{data separation between training phases incurs negligible performance difference} --- while providing operational benefits that standard training cannot offer, including incremental updates and privacy-constrained deployment.
