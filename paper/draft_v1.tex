...
Evaluation across 10 diverse datasets demonstrates that subpopulation specialization consistently outperforms heuristic missing-value handling (Wilcoxon $p = 0.002$, all 10 datasets positive). Our analysis reveals the mechanism: the base model outperforms the combined baseline on the feature-absent population in 100\% of datasets --- a subgroup that single models systematically underserve. The framework simultaneously validates a privacy-preserving collaboration paradigm where institutions share model artifacts rather than data, with all 10 datasets confirming successful cross-phase knowledge transfer.
\end{abstract}
