...
Our analysis reveals why this incremental approach succeeds: under structured missingness, the two-phase design enables population-specific specialization that a single retrained model cannot achieve. The base model outperforms the combined baseline on the feature-absent population in all 10 evaluated datasets, because it learns patterns that a unified model dilutes across both subpopulations. Evaluation across 10 diverse datasets confirms statistically significant improvements (Wilcoxon $p = 0.002$), with adaptive pruning outperforming fixed strategies in 8 of 10 datasets.
\end{abstract}
...
In this work, we introduce an incremental learning framework for GBMs that integrates new features without full retraining, and show it consistently matches or outperforms standard training across 10 diverse datasets (Wilcoxon $p = 0.002$). Our contributions are:
\begin{itemize}
...
\item \textbf{An empirical insight on why incremental learning succeeds under structured missingness:} Our analysis reveals that the base model outperforms the combined baseline on the feature-absent population in all 10 datasets. Under structured missingness, the two-phase design enables population-specific specialization that a single retrained model cannot achieve --- the feature-absent subpopulation is systematically underserved by unified models.
...
\begin{table}[h]
\centering
\caption{Ablation Study: Pruning Mode Comparison. Objective = (combined\textsubscript{ext} -- extended AUC) + (combined\textsubscript{no} -- base AUC). Negative values indicate incremental learning outperforms the combined baseline. Bold marks the best mode per dataset.}
\label{tab:pruning}
\resizebox{0.95\columnwidth}{!}{%
...
Our results demonstrate that the incremental two-phase approach --- which does not require simultaneous access to all data or full retraining --- achieves performance comparable to or better than standard training. Two mechanisms from the incremental design drive this:
...
\textbf{Population specialization as a byproduct of incremental design.} The base model trains on all rows with extended features as NaN, learning patterns specific to the feature-absent subpopulation. This is not an explicit design goal but an emergent benefit: because the base model never sees extended feature values, it specializes naturally on the population that lacks them. Our analysis confirms this outperforms the baseline's base model on the no-extended population in all 10 datasets. Modern GBMs handle this through heuristic default-direction routing, but our results show that dedicated training outperforms heuristic routing under structured missingness.

The broader implication is that incremental learning under structured missingness provides a ``free'' specialization benefit: organizations can update models as new features arrive, avoid full retraining, maintain data separation between phases --- and achieve equal or better performance in the process.
...
The adaptive pruning mechanism --- which determines how much prior knowledge to retain during incremental extension --- achieves the best results in 8 out of 10 datasets, with a built-in safety guarantee that ensures the framework never underperforms training from scratch. Our analysis of 166 experiment configurations reveals why incremental learning succeeds under structured missingness: the two-phase design enables population-specific specialization as an emergent benefit, with the base model outperforming unified baselines on the feature-absent subpopulation in 100\% of datasets.
