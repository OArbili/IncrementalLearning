#!/usr/bin/env python3
"""Run all valid combinations on Movie dataset with heavy augmented nulls.

V2: More aggressive augmentation — 50% null rate, 5 augmented features,
independent null injection per feature to create diverse null patterns.
"""
import itertools
import sys
import os
...
NULL_THRESHOLD = 0.05
MIN_GROUP_PCT = 0.02
AUG_NULL_RATE = 0.50  # 50% of rows will have nulls in each augmented feature
...
# Each feature is independently nulled in 50% of rows (different random indices per feature).
# This creates diverse null patterns with partial overlap.
...
if len(unique_combos) == 0:
    print("\nNo valid combinations found. Exiting.")
    sys.exit(0)
...
for i, combo in enumerate(unique_combos):
    ...
    objective_val = dm.full_run(df.copy(), base_feats, ext_feats, label,
                                 f"movie_aug_v2_combo_{i+1}.csv", n_trials=N_TRIALS)
