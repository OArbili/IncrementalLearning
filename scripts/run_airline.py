...
for bits in itertools.product([0, 1], repeat=len(null_features)):
    ...
    test_df_check = pd.concat([X_test, y_test], axis=1)
    test_df_check['has_extended'] = has_extended[test_df_check.index]

    train_df_temp = df.drop(test_df_check.index)
    train_df_temp['has_extended'] = has_extended[train_df_temp.index]

    train_counts = train_df_temp.groupby([label, 'has_extended']).size()
    test_counts = test_df_check.groupby([label, 'has_extended']).size()
    min_group = min(train_counts.min(), test_counts.min())

    if min_group < 100:
        continue

    test_with = len(test_df_check[test_df_check['has_extended'] == 1])
    test_without = len(test_df_check[test_df_check['has_extended'] == 0])

    if test_with < min_group_size or test_without < min_group_size:
        continue

    ext_names = '+'.join([f.replace(' ', '')[:8] for f in ext_feats])
    valid_combos.append({
        'ext_feats': ext_feats,
...
# --- Filter combos: keep single-feature (except DISTANCE alone) + 3-feature combo ---
REQUIRED_3 = {'TAIL_NUM', 'OP_CARRIER_FL_NUM', 'DISTANCE'}
filtered_combos = []
for combo in unique_combos:
    ext_set = set(combo['ext_feats'])
    # Skip single DISTANCE
    if ext_set == {'DISTANCE'}:
        print(f"  Skipping: {combo['name']} (DISTANCE alone)")
        continue
    # Keep single-feature combos (except DISTANCE) and the 3-feature combo
    if len(ext_set) == 1 or ext_set == REQUIRED_3:
        filtered_combos.append(combo)
unique_combos = filtered_combos

print(f"\nValid: {len(valid_combos)} -> Unique (filtered): {len(unique_combos)}")
print(f"\n{'#':<4} {'Name':<55} {'N_no_ext':<10} {'N_ext':<10} {'%ext':<8} {'Test_no':<10} {'Test_ext':<10} {'MinGrp':<8}")
print("-" * 120)
for i, c in enumerate(unique_combos):
    print(f"{i+1:<4} {c['name']:<55} {c['n_without']:<10} {c['n_with']:<10} {c['pct_ext']:<8} {c['test_without']:<10} {c['test_with']:<10} {c['min_group']:<8}")
...
for i, combo in enumerate(unique_combos):
    ...
    objective_val = dm.full_run(df.copy(), base_feats, ext_feats, label,
                                 f"airline_combo_{i+1}.csv", n_trials=N_TRIALS)
...
print("FINAL SUMMARY")
print(f"{'='*120}")
print(f"{'Combo':<55} {'Objective':<12} {'Base AUC':<10} {'Ext AUC':<10} {'Comb-no':<10} {'Comb+ext':<10} {'N_no_ext':<10} {'N_ext':<10}")
print("-" * 127)
for r in results:
    ...
    print(f"{r['combo']:<55} {obj:<12} {base:<10} {ext:<10} {cno:<10} {cex:<10} {nno:<10} {nex:<10}")
