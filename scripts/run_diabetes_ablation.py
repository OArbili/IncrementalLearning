#!/usr/bin/env python3
"""DiabetesRecord: Feature combo search + ablation study.
...
Usage:
    python run_diabetes_ablation.py
    python run_diabetes_ablation.py --test
"""
import sys
import os
...
RESULTS_DIR = os.path.join(SCRIPTS_DIR, '..', 'results', 'DiabetesRecord')
os.makedirs(RESULTS_DIR, exist_ok=True)

DATASET_NAME = 'DiabetesRecord'
...
def load_diabetes():
    """Diabetes readmission dataset (Natural Nulls)."""
    path = kagglehub.dataset_download("brandao/diabetes")
    csv_path = os.path.join(path, "diabetic_data.csv")
    df = pd.read_csv(csv_path)
    label = "readmitted"
    df[label] = df[label].replace({'NO': 0, '<30': 1, '>30': 1}).astype(int)
    # Drop ID-like and leaky columns (as per notebook)
    df.drop(columns=['encounter_id', 'patient_nbr',
                     'number_inpatient', 'number_emergency',
                     'discharge_disposition_id'], errors='ignore', inplace=True)
