#!/usr/bin/env python3
"""Run all valid combinations on ClientRecord (Telecom Churn) dataset."""
import itertools
import sys
import os
...
# Merge zipcode population with customer churn data (same as notebook)
...
# Target: Customer Status (Stayed=1, else=0)
...
for i, combo in enumerate(unique_combos):
    ...
    objective_val = dm.full_run(df.copy(), base_feats, ext_feats, label,
                                 f"client_record_combo_{i+1}.csv", n_trials=N_TRIALS)
