#!/usr/bin/env python3
"""MovieAugV2: Feature combo search + ablation study.

Augmented dataset: MovieLens 20M with 50% null injection on 5 rating features,
plus natural nulls on tag features (~96%).
...
Usage:
    python run_movieaugv2_ablation.py
    python run_movieaugv2_ablation.py --test
"""
import sys
import os
...
RESULTS_DIR = os.path.join(SCRIPTS_DIR, '..', 'results', 'MovieAugV2')
os.makedirs(RESULTS_DIR, exist_ok=True)

DATASET_NAME = 'MovieAugV2'

# Quick test mode: python run_diabetes_ablation.py --test
...
def load_movieaugv2():
    """MovieLens 20M with augmented nulls (50% per rating feature)."""
    ...
    # Augmentation: inject 50% nulls independently per feature
    rng = np.random.RandomState(SEED)
    augment_features = ['rating_count', 'rating_mean', 'rating_std', 'days_active', 'rating_frequency']
    n_null = int(AUG_NULL_RATE * len(df))

    print(f"\n=== Augmentation: {AUG_NULL_RATE*100:.0f}% null rate per feature ===")
    for feat in augment_features:
        null_idx = rng.choice(df.index, size=n_null, replace=False)
        df.loc[null_idx, feat] = np.nan
        print(f"  {feat}: nulled {len(null_idx)} rows ({df[feat].isna().mean():.2%})")
