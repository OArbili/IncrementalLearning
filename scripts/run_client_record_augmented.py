#!/usr/bin/env python3
"""Run all valid combinations on ClientRecord dataset with augmented nulls."""
import itertools
import sys
import os
...
# These are features with 0% nulls. We'll null them in ~20% of rows that already
# have at least one null in the existing null features (Internet/Phone/Offer group).
rng = np.random.RandomState(SEED)

# Existing null features (Internet group + Phone group + Offer)
existing_null_feats = ['Offer', 'Internet Type', 'Avg Monthly Long Distance Charges']
has_any_null = df[existing_null_feats].isna().any(axis=1)
...
# Augment features: Contract, Monthly Charge, Payment Method
augment_features = ['Contract', 'Monthly Charge', 'Payment Method']
choices = rng.choice(['contract', 'charge', 'payment', 'contract+charge', 'charge+payment', 'all'],
                     size=len(sampled_indices))

contract_idx = sampled_indices[np.isin(choices, ['contract', 'contract+charge', 'all'])]
charge_idx = sampled_indices[np.isin(choices, ['charge', 'contract+charge', 'charge+payment', 'all'])]
payment_idx = sampled_indices[np.isin(choices, ['payment', 'charge+payment', 'all'])]
...
for i, combo in enumerate(unique_combos):
    ...
    objective_val = dm.full_run(df.copy(), base_feats, ext_feats, label,
                                 f"client_record_aug_combo_{i+1}.csv", n_trials=N_TRIALS)
